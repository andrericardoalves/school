...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "school-ear.ear");

        // Current directory is the root of the "school-web" project. Go up one level
        // and enter the "school-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../school-ejb/target/school-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "school-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/school-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "school-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.alves.school.test");
